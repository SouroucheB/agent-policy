#!/usr/bin/env node
// agent-policy generator v1.1.0 — copie autonome ; mise à jour par agent-policy init --upgrade.
...
export function validatePolicy(value, { core = false } = {}) {
  ...
  value.entries.forEach((entry, index) => {
    ...
    keys(entry, ['decision', 'riskClass', 'justification'], label, ['pattern', 'match', 'notMatch', 'residualRisk', 'claudeDeny']);
    const native = Object.hasOwn(entry, 'pattern');
    if (native && (Array.isArray(entry.pattern) === false || entry.pattern.length === 0 || entry.pattern.some(token => typeof token !== 'string' || TOKEN.test(token) === false))) {
...
export function decisionFor(policy, command) {
  ...
  let decision;
  for (const entry of policy.entries) {
    if (entry.pattern && matchesPrefix(entry.pattern, argv) && (decision === undefined || DECISIONS.indexOf(entry.decision) > DECISIONS.indexOf(decision))) decision = entry.decision;
  }
  return decision;
}
...
export function generateCodex(policy) {
  ...
  return header + policy.entries.filter(entry => entry.pattern).map(entry => [
    'prefix_rule(',
