...
export function initProject(root, { upgrade = false } = {}) {
  ...
  const generator = fs.readFileSync(path.join(PACKAGE_ROOT, 'lib/generate.mjs'), 'utf8');
  const previousGenerator = readOptional(root, PROJECT_FILES.generator);
  if (policyText === null) atomicWrite(root, PROJECT_FILES.policy, '{\n  "version": 1,\n  "entries": []\n}\n', 0o644);
  if (previousGenerator === null || (upgrade && previousGenerator !== generator)) atomicWrite(root, PROJECT_FILES.generator, generator, 0o644);
  return previousGenerator !== null && previousGenerator !== generator && upgrade === false
    ? 'Politique conservée ; utiliser init --upgrade pour actualiser le générateur.'
    : `Initialisé : ${PROJECT_FILES.policy}, générateur v${VERSION}.`;
}
