#!/usr/bin/env node
// agent-policy generator v1.2.0 — copie autonome ; mise à jour par agent-policy init --upgrade.
...
function validateAllow(entry, label) {
  ...
  const requireGuard = pattern => {
    if (entry.claudeDeny?.some(guard => guard.pattern === pattern) !== true) fail(`${label} : garde Claude obligatoire : ${pattern}`);
  };
  while (path.posix.basename(entry.pattern[offset] ?? '') === 'rtk') {
    ...
    if (['grep', 'read', 'ls', 'diff', 'gain', 'discover', 'session'].includes(subcommand)) {
      ...
      if (hasClaude && ['grep', 'read', 'diff'].includes(subcommand)) requireGuard(`${root} *.env*`);
      if (hasClaude && subcommand === 'grep') requireGuard(`${root} *--pre*`);
      return;
    }
  ...
  if (hasClaude && ['cat', 'head', 'tail', 'grep', 'rg', 'sed'].includes(basename)) requireGuard(`${guardRoot} *.env*`);
  if (['mkdir', 'cp', 'touch'].includes(basename)) {
    ...
    for (const suffix of ['*.env*', '*..*', '/*', '* /*', '"/*', '* "/*', "'/*", "* '/*", '*=/*', '*~*', '*$*', '*`*', '*\\*']) requireGuard(`${guardRoot} ${suffix}`);
  }
  if (['rg', 'sed'].includes(basename)) {
    if (entry.engines?.length !== 1 || entry.engines[0] !== 'claude' || entry.riskClass !== 'read-only') fail(`${label} : rg/sed en allow réservés à engines ["claude"] avec gardes`);
    if (basename === 'rg') requireGuard(`${guardRoot} *--pre*`);
    if (basename === 'sed') {
      if (subcommand !== '-n') fail(`${label} : seule la lecture sed -n est autorisable`);
      requireGuard(`${guardRoot} *-i*`);
      requireGuard(`${guardRoot} *--in-place*`);
    }
  }
...
export function validatePolicy(value, { core = false } = {}) {
  ...
  value.entries.forEach((entry, index) => {
    ...
    keys(entry, ['decision', 'riskClass', 'justification'], label, ['pattern', 'match', 'notMatch', 'residualRisk', 'claudeDeny', 'claudeAsk', 'engines']);
    ...
    if (native) {
      for (const engine of enginesFor(entry)) {
        const identity = JSON.stringify([engine, entry.pattern]);
    ...
    for (const field of native ? ['match', 'notMatch'] : []) {
    ...
    for (const field of Object.keys(CLAUDE_GUARDS).filter(key => Object.hasOwn(entry, key))) {
      ...
      for (const guard of entry[field]) {
        ...
        if (typeof guard.pattern !== 'string' || /^[A-Za-z0-9_./:@%+,= *<>"'~$`\\\-]+$/u.test(guard.pattern) === false || /^[A-Za-z0-9_./:@%+,=\-]+\*?(?: |$)/u.test(guard.pattern) === false || (native && guard.pattern.startsWith(`${entry.pattern[0]} `) === false)) fail(`${label} : motif Claude avec programme littéral requis`);
        const executableDirectory = ['/bin/*', '/usr/bin/*', '/usr/local/bin/*', '/opt/homebrew/bin/*'].includes(guard.pattern) && entry.decision === 'forbidden';
        const relativeWriteGuard = field === 'claudeDeny' && native && ['mkdir', 'cp', 'touch'].includes(entry.pattern[0]) && entry.engines?.length === 1 && entry.engines[0] === 'claude';
        if (core && executableDirectory === false && relativeWriteGuard === false && guard.pattern.split(' ').some(token => token.includes('/') || token.startsWith('.'))) fail(`${label} : chemin interdit dans une garde du socle global`);
  ...
  for (const entry of expandEntries(policy)) {
    ...
    for (const [examples, expected] of [[entry.match ?? [], true], [entry.notMatch ?? [], false]]) {
      for (const example of examples) {
        ...
        if (enginesFor(entry).includes('claude') && matchesClaude(claudePermission(entry), example) !== expected) fail(`Exemple Claude incohérent : ${JSON.stringify(example)} pour ${entry.pattern.join(' ')}`);
        count += 1;
      }
    }
...
export function decisionFor(policy, command, engine = 'codex') {
  ...
  for (const entry of entriesFor(policy, engine)) {
    ...
    const matches = entry.pattern && (engine === 'codex' ? matchesPrefix(entry.pattern, argv) : matchesClaude(claudePermission(entry), command, { shellText: true }));
...
export function generatePermissions(policy) {
  ...
  for (const entry of entriesFor(policy, 'claude')) {
    if (entry.pattern) permissions[CLAUDE_KEYS[entry.decision]].push(claudePermission(entry));
...
export function assertClaudeAllowPatterns(permissions) {
  for (const permission of permissions.allow) {
    const pattern = permission.slice(5, -1);
    if (pattern.includes('*') && (pattern.indexOf('*') !== pattern.length - 1 || /(?: |:)\*$/u.test(pattern) === false)) fail(`Claude : pas de joker interne dans un allow : ${permission}`);
  }
}
