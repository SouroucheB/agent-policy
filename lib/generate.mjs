...
export function expandEntries(policy) {
  return policy.entries.flatMap(entry => [entry, ...(policy.commandPrefixes ?? []).map(prefix => {
    ...
    return {
      ...
      ...(entry.claudeDeny ? { claudeDeny: entry.claudeDeny.map(guard => ({
...
export function validatePolicy(value, { core = false } = {}) {
  ...
  value.entries.forEach((entry, index) => {
    ...
    keys(entry, ['decision', 'riskClass', 'justification'], label, ['pattern', 'match', 'notMatch', 'residualRisk', 'claudeDeny', 'engines']);
    ...
    if (Object.hasOwn(entry, 'claudeDeny')) {
      ...
      for (const guard of entry.claudeDeny) {
        ...
        if (core && executableDirectory === false && guard.pattern.split(' ').some(token => token.includes('/') || token.startsWith('.'))) fail(`${label} : chemin interdit dans une garde du socle global`);
...
export function validateExamples(policy) {
  ...
  for (const entry of expandEntries(policy)) {
    ...
    for (const guard of enginesFor(entry).includes('claude') ? entry.claudeDeny ?? [] : []) {
