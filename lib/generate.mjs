...
export const REFUSAL_RISK_CLASSES = [
  'destructive-program', 'remote-publication', 'cloud-mutation', 'paid-provider',
  'shared-local-state', 'arbitrary-execution',
];
export const ALLOW_RISK_CLASSES = ['read-only', 'local-reversible'];
export const RISK_CLASSES = [...REFUSAL_RISK_CLASSES, ...ALLOW_RISK_CLASSES];
...
export const GIT_C_READS = ['status', 'diff', 'log', 'show', 'rev-parse', 'ls-files', 'grep', 'merge-tree'];
...
// Table figée le 2026-09-20, listes complètes et version Git dans
// test/fixtures/git-long-options.json (git <commande> --git-completion-helper-all).
// https://git-scm.com/docs/gitcli : le parseur accepte les préfixes uniques.
// optionSet désigne le parseur, pas l'union des options de plusieurs parseurs :
// diff.c est aussi appelé par log, show, stash list et blame.
// Gardes complètes inchangées ; les préfixes ci-dessous exigent toujours ask.
export const GIT_OPTION_PREFIXES = [
  { commands: ['fetch'], optionSet: 'fetch', option: '--upload-pack', prefix: '--upl', reason: '--up est partagé avec --update-head-ok et --update-shallow ; --unshallow reste distinct.' },
  { commands: ['grep'], optionSet: 'grep', option: '--open-files-in-pager', prefix: '--op', reason: '--o est partagé avec --only-matching et --or.' },
  { commands: ['diff', 'log', 'show', 'stash list', 'blame'], optionSet: 'diff', option: '--ext-diff', prefix: '--ext', reason: '--ex est partagé avec --exit-code.' },
  { commands: ['branch'], optionSet: 'branch', option: '--delete', prefix: '--d', reason: 'Seule option commençant par d.' },
  { commands: ['branch'], optionSet: 'branch', option: '--move', prefix: '--mo', reason: '--m est partagé avec --merged.' },
  { commands: ['branch'], optionSet: 'branch', option: '--copy', prefix: '--cop', reason: '--co est partagé avec --color, --column et --contains.' },
  { commands: ['branch'], optionSet: 'branch', option: '--force', prefix: '--forc', reason: '--for est partagé avec --format.' },
  { commands: ['branch'], optionSet: 'branch', option: '--create-reflog', prefix: '--cr', reason: '--c est partagé avec --color, --column, --contains et --copy.' },
  { commands: ['branch'], optionSet: 'branch', option: '--edit-description', prefix: '--e', reason: 'Seule option commençant par e ; --edit de add/commit reste de confiance.' },
  { commands: ['branch'], optionSet: 'branch', option: '--set-upstream-to', prefix: '--set-upstream-', reason: '--set-upstream est une autre option exacte, obsolète ; --s est aussi partagé avec --show-current et --sort.' },
  { commands: ['branch'], optionSet: 'branch', option: '--unset-upstream', prefix: '--u', reason: 'Seule option commençant par u.' },
  { commands: ['diff', 'log', 'show', 'stash list', 'blame'], optionSet: 'diff', option: '--output', prefix: null, reason: 'Tout préfixe plus court que --output est ambigu avec les trois --output-indicator-* (--output-indicator-new, --output-indicator-old, --output-indicator-context), donc refusé par git ; aucune abréviation n’existe. Motifs bornés seulement, dans toutes les gardes Git.' },
  // git.c handle_options compare les noms complets, sans parse-options :
  // https://github.com/git/git/blob/v2.50.1/git.c
  { commands: [], optionSet: null, option: '--exec-path', prefix: null, reason: 'Option globale reconnue par son nom complet dans git.c, pas par abréviation.' },
  { commands: [], optionSet: null, option: '--config-env', prefix: null, reason: 'Option globale reconnue par son nom complet dans git.c, pas par abréviation.' },
];
// Catalogue figé le 2026-09-20 : git 2.50.1 (Apple Git-155),
// git help -a --no-aliases --no-external-commands (hors interfaces documentaires),
// plus lfs et svn. Capture de référence : test/fixtures/git-help-all.txt.
// Embarqué ici pour que le générateur copié par init reste autonome et hors ligne.
export const GIT_SUBCOMMANDS = [
  'add', 'am', 'annotate', 'apply', 'archimport', 'archive',
  'backfill', 'bisect', 'blame', 'branch', 'bugreport', 'bundle',
  'cat-file', 'check-attr', 'check-ignore', 'check-mailmap', 'check-ref-format', 'checkout',
  'checkout-index', 'cherry', 'cherry-pick', 'citool', 'clean', 'clone',
  'column', 'commit', 'commit-graph', 'commit-tree', 'config', 'count-objects',
  'credential', 'credential-cache', 'credential-store', 'cvsexportcommit', 'cvsimport', 'cvsserver',
  'daemon', 'describe', 'diagnose', 'diff', 'diff-files', 'diff-index',
  'diff-pairs', 'diff-tree', 'difftool', 'fast-export', 'fast-import', 'fetch',
  'fetch-pack', 'filter-branch', 'fmt-merge-msg', 'for-each-ref', 'for-each-repo', 'format-patch',
  'fsck', 'gc', 'get-tar-commit-id', 'gitk', 'gitweb', 'grep',
  'gui', 'hash-object', 'help', 'hook', 'http-backend', 'imap-send',
  'index-pack', 'init', 'instaweb', 'interpret-trailers', 'lfs', 'log',
  'ls-files', 'ls-remote', 'ls-tree', 'mailinfo', 'mailsplit', 'maintenance',
  'merge', 'merge-base', 'merge-file', 'merge-index', 'merge-one-file', 'merge-tree',
  'mergetool', 'mktag', 'mktree', 'multi-pack-index', 'mv', 'name-rev',
  'notes', 'p4', 'pack-objects', 'pack-redundant', 'pack-refs', 'patch-id',
  'prune', 'prune-packed', 'pull', 'push', 'quiltimport', 'range-diff',
  'read-tree', 'rebase', 'reflog', 'refs', 'remote', 'repack',
  'replace', 'replay', 'request-pull', 'rerere', 'reset', 'restore',
  'rev-list', 'rev-parse', 'revert', 'rm', 'scalar', 'send-email',
  'send-pack', 'sh-i18n', 'sh-setup', 'shortlog', 'show', 'show-branch',
  'show-index', 'show-ref', 'sparse-checkout', 'stash', 'status', 'stripspace',
  'submodule', 'svn', 'switch', 'symbolic-ref', 'tag', 'unpack-file',
...
// Toute commande du catalogue non explicitement autorisée reçoit des gardes ask.
export const GIT_C_MUTATIONS = GIT_SUBCOMMANDS.filter(command => !GIT_C_COMMANDS.includes(command));
...
export function gitCMutationPatterns(root) {
  return [...GIT_C_MUTATIONS.flatMap(command => [
    `${root} * ${command}`, `${root} * ${command} *`,
  ]), `${root} rm *`];
}
export function optionGuardPatterns(root, option) {
  // Exception validée : --output* attraperait --output-indicator-new/old/context.
  if (option === '--output') return ['', ' *'].flatMap(middle =>
    ['', '=*', ' *'].map(suffix => `${root}${middle} ${option}${suffix}`));
  return [`${root} ${option}*`, `${root} * ${option}*`];
}
export function gitAbbreviationGuardPatterns(root, command) {
  return GIT_OPTION_PREFIXES.filter(row => row.prefix && row.commands.includes(command))
    .flatMap(row => optionGuardPatterns(root, row.prefix));
}
...
function validateAllow(entry, label) {
  ...
  if (basename === 'git' && hasClaude) {
    const root = entry.pattern.join(' ');
    const command = entry.pattern.slice(offset + 1).join(' ');
    const guards = [...(entry.claudeAsk ?? []), ...(entry.claudeDeny ?? [])];
    for (const row of GIT_OPTION_PREFIXES.filter(row => row.prefix && row.commands.includes(command))) {
      if (!optionGuardPatterns(root, row.option).some(pattern => guards.some(guard => guard.pattern === pattern))) continue;
      for (const pattern of optionGuardPatterns(root, row.prefix)) {
        requireGuard(pattern, 'claudeAsk');
        if (entry.claudeDeny?.some(guard => guard.pattern === pattern)) fail(`${label} : abréviation Git en ask, jamais deny : ${pattern}`);
      }
    }
  }
  ...
  if (['rg', 'sed'].includes(basename)) {
    if (entry.engines?.length !== 1 || entry.riskClass !== 'read-only') fail(`${label} : rg/sed exigent un moteur explicite et read-only`);
    ...
    if (basename === 'sed') {
      ...
      if (hasClaude) {
        ...
        if (entry.pattern.length !== offset + 2) fail(`${label} : préfixe Codex sed -n exact requis`);
        if (!/(?:^|[^\w-])-i(?:$|[^\w-])/u.test(entry.residualRisk ?? '') || !entry.residualRisk?.includes('--in-place')) fail(`${label} : residualRisk sed -i / --in-place requis pour Codex`);
      }
    }
  }
  if (basename === 'awk') {
...
export function validatePolicy(value, { core = false } = {}) {
  ...
  value.entries.forEach((entry, index) => {
    ...
    const compatibleRisks = entry.decision === 'allow' ? ALLOW_RISK_CLASSES : REFUSAL_RISK_CLASSES;
    if (compatibleRisks.includes(entry.riskClass) === false) fail(`${label} : risque incompatible avec ${entry.decision} : ${entry.riskClass}`);
...
export function assertClaudeAllowPatterns(permissions) {
  ...
  for (const permission of permissions.allow) {
    ...
    if (pattern.includes('*') && (pattern.indexOf('*') !== pattern.length - 1 || /(?: |:)\*$/u.test(pattern) === false)) {
      ...
      if (!valid) fail(`Claude : pas de joker interne dans un allow sans exception git -C gardée : ${permission}`);
      if (checkedRoots.has(root)) continue;
      const abbreviations = GIT_C_COMMANDS.flatMap(command => gitAbbreviationGuardPatterns(`${root} * ${command}`, command));
      for (const guard of [...gitCMutationPatterns(root), ...GIT_C_OPTIONS.flatMap(option => optionGuardPatterns(root, option)), ...abbreviations]) {
        const rule = `Bash(${guard})`;
        if (!ask.has(rule) || deny.has(rule)) fail(`Claude : pas de joker interne dans un allow sans exception git -C gardée complète, garde ask obligatoire (jamais deny) : ${rule}`);
      }
