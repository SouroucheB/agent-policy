...
import { WRITE_GUARD_FILE, generateWriteGuardSettings } from './write-boundary.mjs';
...
export function initProject(root, { upgrade = false } = {}) {
  ...
  const previousSettings = readOptional(root, PROJECT_FILES.claude);
  const settings = generateWriteGuardSettings(previousSettings);
  const source = fs.readFileSync(path.join(PACKAGE_ROOT, 'lib/generate.mjs'), 'utf8');
  const marker = 'const EMBEDDED_CORE_TEXT = null;';
  if (source.split(marker).length !== 2) throw new Error('Marqueur du socle embarqué absent ou dupliqué');
  const generator = source.replace(marker, () => `const EMBEDDED_CORE_TEXT = ${JSON.stringify(snapshot.text)};`);
  const copies = [
    [PROJECT_FILES.generator, generator],
    [PROJECT_FILES.brief, fs.readFileSync(path.join(PACKAGE_ROOT, 'lib/brief.mjs'), 'utf8')],
    [PROJECT_FILES.core, snapshot.text],
    [PROJECT_FILES.coreVersion, snapshot.metadata],
    [WRITE_GUARD_FILE, fs.readFileSync(path.join(PACKAGE_ROOT, 'lib/claude-worktree-write-guard.cjs'), 'utf8')],
  ].map(([relative, content]) => ({ relative, content, before: readOptional(root, relative) }));
  ...
  if (settings !== previousSettings) atomicWrite(root, PROJECT_FILES.claude, settings, previousSettings === null ? 0o644 : fs.statSync(safePath(root, PROJECT_FILES.claude)).mode & 0o777);
  return copies.some(copy => copy.before !== null && copy.before !== copy.content) && upgrade === false
    ? 'Politique conservée ; utiliser init --upgrade pour actualiser le socle, le générateur, brief et le garde d’écriture.'
    : `Initialisé : ${PROJECT_FILES.policy}, socle, générateur, brief et garde d’écriture v${VERSION}.`;
}

export function readCore() {
  return bundledCoreSnapshot().policy;
}
...
export function unifiedDiff(relative, before, after) {
  ...
  let oldLines = before?.match(/[^\n]*\n|[^\n]+$/gu) ?? [];
  let newLines = after?.match(/[^\n]*\n|[^\n]+$/gu) ?? [];
  const render = (prefix, line) => line.endsWith('\n') ? prefix + line.slice(0, -1) : `${prefix}${line}\n\\ No newline at end of file`;
  const header = `--- ${before === null ? '/dev/null' : relative}\n+++ ${after === null ? '/dev/null' : relative}\n`;
  const hunk = (startOld, startNew, removed, added, changes, simplified = false) =>
    `@@ -${removed === 0 ? startOld : startOld + 1},${removed} +${added === 0 ? startNew : startNew + 1},${added} @@${simplified ? ' Diff simplifié : remplacement du milieu' : ''}\n${changes.join('\n')}`;

  // Retirer les extrémités communes avant de borner le coût de la LCS.
  let offset = 0;
  while (offset < oldLines.length && offset < newLines.length && oldLines[offset] === newLines[offset]) offset += 1;
  let oldEnd = oldLines.length;
  let newEnd = newLines.length;
  while (oldEnd > offset && newEnd > offset && oldLines[oldEnd - 1] === newLines[newEnd - 1]) { oldEnd -= 1; newEnd -= 1; }
  oldLines = oldLines.slice(offset, oldEnd);
  newLines = newLines.slice(offset, newEnd);
  if (oldLines.length === 0 && newLines.length === 0) return header;
  const simplified = oldLines.length * newLines.length > 4_000_000;
  if (simplified || oldLines.length === 0 || newLines.length === 0) {
    // Affichage complet, sans LCS ni troncature : retraits puis ajouts.
    const changes = [...oldLines.map(line => render('-', line)), ...newLines.map(line => render('+', line))];
    return header + hunk(offset, offset, oldLines.length, newLines.length, changes, simplified) + '\n';
  }
  const width = newLines.length + 1;
  const cells = new Uint32Array((oldLines.length + 1) * width);
  for (let i = oldLines.length - 1; i >= 0; i -= 1) {
    for (let j = newLines.length - 1; j >= 0; j -= 1) {
      cells[i * width + j] = oldLines[i] === newLines[j] ? cells[(i + 1) * width + j + 1] + 1 : Math.max(cells[(i + 1) * width + j], cells[i * width + j + 1]);
    }
  }
  let i = 0;
  let j = 0;
  ...
  while (i < oldLines.length || j < newLines.length) {
    ...
    while ((i < oldLines.length || j < newLines.length) && (i >= oldLines.length || j >= newLines.length || oldLines[i] !== newLines[j])) {
      if (j < newLines.length && (i === oldLines.length || cells[i * width + j + 1] >= cells[(i + 1) * width + j])) { changes.push(render('+', newLines[j])); j += 1; }
      else { changes.push(render('-', oldLines[i])); i += 1; }
    }
    hunks.push(hunk(offset + startOld, offset + startNew, i - startOld, j - startNew, changes));
  }
