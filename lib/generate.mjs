...
export const GIT_C_MUTATIONS = [
  'push', 'pull', 'fetch', 'merge', 'rebase', 'reset', 'restore', 'checkout', 'switch',
  'commit', 'add', 'stash', 'clean', 'rm', 'tag', 'config', 'gc',
...
function validateAllow(entry, label) {
  ...
  if (entry.claudePattern) {
    ...
    if (GIT_C_READS.some(read => [`git -C * ${read}`, `git -C * ${read} *`].includes(direct))) return;
    fail(`${label} : claudePattern allow réservé aux formes fermées uniq et lectures git -C`);
  }
...
export function assertClaudeAllowPatterns(permissions) {
  for (const permission of permissions.allow) {
    ...
    if (pattern.includes('*') && (pattern.indexOf('*') !== pattern.length - 1 || /(?: |:)\*$/u.test(pattern) === false)) {
      ...
      const valid = GIT_C_READS.some(read => [`git -C * ${read}`, `git -C * ${read} *`].includes(direct));
