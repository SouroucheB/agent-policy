...
test('session Codex courante : lectures confinées sans règle, filtres explicitement autorisés', () => {
  ...
  for (const prefix of prefixes) {
    for (const command of [
      'sed 1,20p AGENTS.md', 'git branch --list', 'git branch --list --no-list -D feature',
      'sed -i 1d file', "awk '{print $1}' file", 'uniq -c',
    ]) assert.equal(verdict(prefix + command), undefined, prefix + command);
    for (const command of ['rg -n foo src', 'rg --pre program needle src', 'git diff --stat', 'git log --oneline -5', 'sed -n 1,20p AGENTS.md', "sed -n '1770,1790p' AGENTS.md", 'cat README.md', 'ls -la docs']) assert.equal(verdict(prefix + command), 'allow');
